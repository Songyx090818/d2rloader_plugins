...
//  disassembled before being relied on. Full-body byte witnesses guard the
...
//  delete (9, itemId) at D2RCore.dll+0x3D7120, then SetEvent. The gear refresh
...
//      0x48B890  inline hook. Only the call returning to D2RCore+0x3D7126 is
//          converted; the skill id is the flush record's skill D2RCore itself
//          loaded into [rsp+0x58] at +0x3D70E2. The 147-byte flush block and
...
//          +08  int32  value
//
//  Proven by D2RCore!ReadWideUnitStat (export +0x7A99D0 -> core +0x3E2E40,
//  key built at +0x3E2E98, 16-byte binary search at +0x3E2EEF, value read at
//  +0x3E2F20), checked byte for byte at load. D2RCore's attach sets the child
//  list's parent at [list+78h] (+0x3E4818), so STATLIST_GetOwner (0x2F8120)
//  still names the player, and RemoveWideDeathStats skips item lists
//  (+0x3E501C), so charms stay attached through death.
//
...
constexpr std::uint64_t CoreFlushWindowRva          = 0x3D70BA;
constexpr std::uint64_t CoreFlushActivateCallRva    = 0x3D7107;  // call [slot] -> 0x581970
constexpr std::uint64_t CoreFlushDeleteCallRva      = 0x3D7120;  // call [slot] -> 0x48B890
constexpr std::uint64_t CoreFlushSetEventCallRva    = 0x3D7147;  // call [slot] -> 0x48B720
constexpr std::uint64_t CoreFlushDeleteReturnRva    = 0x3D7126;
constexpr std::size_t   CoreFlushSkillSlot          = 0x58;      // [rsp+58h] at the call
constexpr std::uint64_t CoreReadWideUnitStatRva     = 0x7A99D0;  // export, calls the core below
constexpr std::uint64_t CoreWideStatLayoutRva       = 0x3E2E56;  // ReadWideUnitStat core body
...
constexpr auto ActivateItemAuraBody = std::to_array<std::uint8_t>({
    ...
    0x48, 0x8B, 0xCF, 0x8B, 0xD8, 0xE8, 0xF0, 0x9A, 0x8A, 0x03, 0xC7, 0x44,
    ...
    0x91, 0xEB, 0xFF, 0x8B, 0xD3, 0x48, 0x8B, 0xCF, 0xE8, 0xBD, 0x9A, 0x8A,
...
constexpr auto ItemAuraEventBody = std::to_array<std::uint8_t>({
    ...
    0x8B, 0xCE, 0x44, 0x8B, 0xE8, 0xE8, 0x48, 0x42, 0x9F, 0x03, 0xBA, 0x00,
    ...
    0x05, 0x00, 0x41, 0x8B, 0xD5, 0x48, 0x8B, 0xCE, 0xE8, 0x55, 0x41, 0x9F,
...
constexpr auto CoreWideStatLayoutWindow = std::to_array<std::uint8_t>({
    ...
    0xAE, 0x9C, 0x29, 0x00, 0x48, 0x89, 0xC1, 0x31, 0xC0, 0x48, 0x85, 0xC9,
...
auto CheckExact(std::uint64_t rva, const std::uint8_t* bytes, std::size_t size, const char* name) noexcept -> bool {
    ...
    LogError("AuraReactivation: %s at RVA 0x%llX does not match D2RLoader 1.3.0.", name,
...
auto ValidateCoreStatStore() noexcept -> bool {
    ...
    if (!CoreWindowMatches(image, imageSize, CoreReadWideUnitStatRva, CoreReadWideUnitStatExport.data(),
        ...
        LogError("AuraReactivation: D2RCore.dll's stat store is not the 1.3.0 layout this plugin reads; "
...
auto ResolveCoreFlushReturn() noexcept -> std::uintptr_t {
    ...
    if (!CoreWindowMatches(image, imageSize, CoreFlushWindowRva, CoreFlushWindow.data(), CoreFlushWindow.size())) {
        LogWarn("AuraReactivation: D2RCore.dll is not the 1.3.0 build this plugin was verified against; "
...
D2RL_PLUGIN_EXPORT auto D2RLoaderLoadPlugin(const D2RL::PluginContext* context) noexcept -> bool {
    ...
    if (!ValidateEventLayout()) {
        LogError("AuraReactivation: the event layout does not match D2RLoader 1.3.0; nothing was changed.");
        return true;
    }
