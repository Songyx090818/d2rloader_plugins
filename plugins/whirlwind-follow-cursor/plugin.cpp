...
#
# Seven parts, each behind its own switch:
...
void LoadConfiguration(const D2RL::PluginContext* context) noexcept {
	...
	D2RL::LogInfoF(
		context,
		"config: enabled=%d ctc=%d chance=%d allow_list=%d proc_fix=%d dual_wield=%d cyclone=%d grace_ms=%d keep_heading=%d radius=%d mana_frames=%d spin_melee=%d death=%d potions=%d bows=%d",
		...
		g_config.nearCursorRadius, g_config.manaIntervalFrames, g_config.spinAtMeleeTargets, g_config.deathWhileWhirlwinding, g_config.potionsWhileWhirlwinding, g_config.whirlwindWithBows);
}
...
constexpr const char*   SourceTag            = "celestialrayone.whirlwind";
...
struct Native {
	...
	std::int32_t (*pathSetVelocity)(void*, std::int32_t, const char*, std::int32_t) = nullptr; // 00342AE0
	...
	std::int32_t (*whirlwindVelocity)(void*)                            = nullptr; // 00438180
	...
	std::uint8_t (*unitDataContext)(void*)                              = nullptr; // 0034A0E0
	std::int32_t (*unitClassId)(void*)                                  = nullptr; // 00349860
	void* (*dataTables)(std::uint8_t)                                   = nullptr; // 00300A90
...
void BindNatives(std::uintptr_t base) noexcept {
	...
	Bind(g_native.pathSetVelocity, base, 0x342AE0);
	...
	Bind(g_native.whirlwindVelocity, base, 0x438180);
	...
	Bind(g_native.unitDataContext, base, 0x34A0E0);
	Bind(g_native.unitClassId, base, 0x349860);
	Bind(g_native.dataTables, base, 0x300A90);
...
void ServerRetarget(void* unit, void* skill, std::int32_t x, std::int32_t y) noexcept {
	...
	if (g_native.serverPathCompute(path, unit, 0, 1) != 0) {
		g_native.pathSetVelocity(path, g_native.whirlwindVelocity(unit), SourceTag, 1104);
	}
...
void ClientRetarget(void* unit, void* skill, std::int32_t x, std::int32_t y) noexcept {
	...
	if (g_native.clientPathCompute(unit, path) != 0) {
		// Player velocity exactly as 00230BCA..00230CA3 reads it.
		const std::uint8_t context = g_native.unitDataContext(unit);
		const std::int64_t classId = g_native.unitClassId(unit);
		void* const        tables  = g_native.dataTables(context);
		if (tables != nullptr && classId >= 0) {
			const auto* t     = static_cast<std::uint8_t*>(tables);
			std::uint64_t count = 0;
			std::uintptr_t rows = 0;
			std::memcpy(&count, t + 0x1248, sizeof(count));
			std::memcpy(&rows, t + 0x1240, sizeof(rows));
			if (rows != 0 && static_cast<std::uint64_t>(classId) < count) {
				const std::uint8_t velocity = reinterpret_cast<std::uint8_t*>(rows + static_cast<std::uintptr_t>(classId) * 0xD0)[0x3C];
				g_native.pathSetVelocity(path, static_cast<std::int32_t>(velocity) << 8, SourceTag, 1360);
			}
		}
	}
	FinishRetarget(unit, skill, path, room, pattern);
...
constexpr D2RL::PluginInfo WhirlwindInfo {
	...
	.description = "Whirlwind rework: procs while Uninterruptible, proc start fix, dual wield same target, cyclone steering, death and potions while Whirlwinding, Whirlwind with bows.",
